...
const RightPanel: React.FC<RightPanelProps> = ({ 
  ...
  return (
    <aside 
        style={{ width: width }}
        className={`bg-white border-l border-gray-200 flex flex-col flex-shrink-0 overflow-hidden shadow-lg z-10 relative ${isResizing ? '' : 'transition-all duration-300 ease-in-out'}`}
    >
      <div className="absolute top-0 left-0 w-1 h-full cursor-col-resize hover:bg-blue-500 z-20" onMouseDown={onResizeStart} />
      <div className="p-4 border-b border-gray-200 min-w-[20rem] flex-shrink-0">
            ...
            <section className="border-t pt-6">
               ...
               <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-6">
                   <div className="flex items-center gap-2">
                      <input type="checkbox" id="prop-visible" checked={formData.visible ?? true} onChange={(e) => handleGenericChange('visible', e.target.checked)} className="h-4 w-4" />
                      <label htmlFor="prop-visible" className="text-sm text-gray-700">Visible</label>
                   </div>
                   <div className="flex items-center gap-2">
                      <input type="checkbox" id="prop-billboard" checked={formData.alwaysFacingUser ?? false} onChange={(e) => handleGenericChange('alwaysFacingUser', e.target.checked)} className="h-4 w-4" />
                      <label htmlFor="prop-billboard" className="text-sm text-gray-700">Billboard</label>
                   </div>
               </div>
