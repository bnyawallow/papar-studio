...
const TargetPlane = ({ target, onDeselect }: { target: Target, onDeselect: () => void }) => {
  ...
  return (
    <mesh onClick={(e) => { e.stopPropagation(); onDeselect(); }} rotation={[-Math.PI / 2, 0, 0]}>
...
interface ScenePanelProps {
  ...
  sceneSettings?: SceneSettings; // New prop
}
...
const ScenePanel: React.FC<ScenePanelProps> = ({ 
  ...
  // Default camera for editing
  const defaultCameraPos: [number, number, number] = [0, 8, 12];
  // Closer, more direct camera for preview to simulate phone view better
  const previewCameraPos: [number, number, number] = [0, 0, 14];
  ...
  return (
    <main 
      className={`flex-1 relative ${isPreviewMode ? 'w-full h-full bg-transparent' : 'bg-gray-600'}`}
    ...
    >
      ...
      {!isPreviewMode && (
        <div className="absolute top-2 left-2 z-20 bg-gray-800 p-1 rounded-md flex gap-1 shadow-lg">
            <button onClick={() => setTransformMode('translate')} className={`p-2 rounded ${transformMode === 'translate' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'}`} disabled={isRunning}><MoveIcon className="w-4 h-4" /></button>
            <button onClick={() => setTransformMode('rotate')} className={`p-2 rounded ${transformMode === 'rotate' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'}`} disabled={isRunning}><RotateIcon className="w-4 h-4" /></button>
            <button onClick={() => setTransformMode('scale')} className={`p-2 rounded ${transformMode === 'scale' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'}`} disabled={isRunning}><ScaleIcon className="w-4 h-4" /></button>
            <div className="w-px bg-gray-600 mx-1"></div>
            <button onClick={() => setSnapEnabled(!snapEnabled)} className={`p-2 rounded ${isSnapping ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'}`} title={`Toggle Snap (Hold Shift) - ${isSnapping ? 'ON' : 'OFF'}`} disabled={isRunning}><MagnetIcon className="w-4 h-4" /></button>
            <div className="w-px bg-gray-600 mx-1"></div>
            <button onClick={() => setIsRunning(!isRunning)} className={`px-3 py-2 rounded flex items-center gap-2 text-sm font-bold transition-colors ${isRunning ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-green-600 text-white hover:bg-green-700'}`}>{isRunning ? 'STOP' : 'RUN SCRIPT'}</button>
        </div>
      )}
      ...
      >
        ...
        <directionalLight 
            position={sceneSettings?.directionalLightPosition ?? [10, 10, 5]} 
            intensity={sceneSettings?.directionalLightIntensity ?? 1.5} 
            castShadow 
        />
        {!isPreviewMode && sceneSettings?.showGrid && <gridHelper args={[20, 20]} />}
        {!isPreviewMode && sceneSettings?.showAxes && <axesHelper args={[10]} />}
        {target && (target.visible ?? true) && (
          <>
