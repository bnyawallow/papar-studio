...
export class Policies {
  ...
  public readField<V = StoreValue>(
    ...
    if (options.typename === void 0) {
      const typename = context.store.getFieldValue(objectOrReference, "__typename") as string;
      if (typename) options.typename = typename;
    }

    const storeFieldName = this.getStoreFieldName(options);
    const fieldName = fieldNameFromStoreName(storeFieldName);
    const existing = context.store.getFieldValue(objectOrReference, storeFieldName) as SafeReadonly<V>;
...
function makeFieldFunctionOptions(
  ...
  return {
    ...
    readField<T = any>(
      ...
      return policies.readField(options, context) as unknown as T;
    },
...
function computeKeyObject(
  ...
  // so we are careful to build keyObj in the order
  // A function whose declared type is neither 'undefined', 'void', nor 'any' must return a value.
  const keyObj = Object.create(null);
  specifier.forEach(s => {
      if (Array.isArray(s)) {
          const nested = computeKeyObject(response, s, strict, aliasMap);
          if (Object.keys(nested).length > 0) {
              // For now, nested key objects are just stringified and added as a
              // property of the parent key object.
              // TODO What if the key contains special characters?
              keyObj[JSON.stringify(s)] = nested;
          }
      } else {
          const key = aliasMap && aliasMap.aliases && aliasMap.aliases[s] || s;
          const value = response[key];
          if (value !== void 0) {
              keyObj[s] = value;
          } else if (strict) {
              throw new InvariantError(`Missing field '${key}' in ${JSON.stringify(response)}`);
          }
      }
  });
  return keyObj;
