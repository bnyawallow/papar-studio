...
interface ReadContext extends ReadMergeModifyContext {
  ...
  path: (string | number)[];
  clientOnly: boolean;
// Fix: Add missing properties from ReadMergeModifyContext
...
function missingFromInvariant(
  ...
  return new MissingFieldError(
    ...
    context.query,
    context.clientOnly,
// Fix: Property 'variables' does not exist on type 'ReadContext'.
    context.variables,
  );
}
...
export class StoreReader {
  ...
  public diffQueryAgainstStore<T>({
    ...
    const execResult = this.executeSelectionSet({
      ...
      context: {
// Fix: Object literal may only specify known properties, and 'store' does not exist in type 'ReadContext'.
        store,
        query,
        policies,
        variables,
...
// Fix: Property 'store' does not exist on type 'ReadContext'.
      if (supportsResultCaching(context.store)) {
// Fix: Property 'store' does not exist on type 'ReadContext'.
        return context.store.makeCacheKey(
          selectionSet,
          isReference(parent) ? parent.__ref : parent,
// Fix: Property 'varString' does not exist on type 'ReadContext'.
          context.varString,
        );
      }
    }
...
// Fix: Property 'name' does not exist on type 'SelectionNode'.
          fieldName: (selection as FieldNode).name.value,
// Fix: Type 'SelectionNode' is not assignable to type 'FieldNode'.
...
// Fix: Property 'alias' does not exist on type 'SelectionNode'.
        const resultName = resultKeyNameFromField(selection as FieldNode);
...
// Fix: Property 'directives' does not exist on type 'SelectionNode'.
          (selection as FieldNode).directives &&
// Fix: Property 'directives' does not exist on type 'SelectionNode'.
          (selection as FieldNode).directives!.some(d => d.name.value === "client")
        );

        if (fieldValue === void 0) {
          if (!addTypenameToDocument.added(selection as FieldNode)) {
            getMissing().push(
...
// Fix: Property 'selectionSet' does not exist on type 'SelectionNode'.
        } else if (!(selection as FieldNode).selectionSet) {
...
// Fix: Property 'store' does not exist on type 'ReadContext'.
              context.store,
// Fix: Argument of type 'SelectionNode' is not assignable to parameter of type 'FieldNode'.
...
// Fix: Property 'selectionSet' does not exist on type 'SelectionNode'.
            selectionSet: (selection as FieldNode).selectionSet!,
