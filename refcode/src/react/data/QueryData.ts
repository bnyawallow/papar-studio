...
// Fix: Import DocumentNode and TypedDocumentNode
...
> {
  ...
  private startQuerySubscription(onNewData: () => void = this.onNewData) {
    ...
    // Fix: Cast `this.currentObservable` to `Observable` to resolve subscription-related type errors.
    this.currentSubscription = (this.currentObservable! as unknown as Observable<QueryResult<TData, TVariables>>).subscribe({
  ...
  private removeObservable(andDelete: boolean) {
    if (this.currentObservable) {
      (this.currentObservable as any)["tearDownQuery"]();
      if (andDelete) {
